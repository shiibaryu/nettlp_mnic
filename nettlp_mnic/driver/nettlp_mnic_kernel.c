...
static int mnic_setup_tx_resource(struct mnic_ring *tx_ring,int i)
{
	int size;
...
{	
	...
	for(i=0; i < adapter->num_tx_queues; i++){
		ret = mnic_setup_tx_resource(adapter->tx_ring[i],i);
		if(ret){
...
int mnic_setup_rx_resource(struct mnic_ring *rx_ring,int i)
{
	int size;
	...
	info("rx desc base %d is %d",i,adapter->bar4->rx_desc_base[i]);
...
static int mnic_setup_all_rx_resources(struct mnic_adapter *adapter)
{
	...
	for(i=0;i<adapter->num_rx_queues;i++){
		ret = mnic_setup_rx_resource(adapter->rx_ring[i],i);
		if(ret){
...
static int __mnic_maybe_stop_tx(struct mnic_ring *tx_ring,uint16_t size)
{
	...
	netif_stop_subqueue(ndev,tx_ring->queue_index);
	...
	netif_wake_subqueue(ndev,tx_ring->queue_index);
